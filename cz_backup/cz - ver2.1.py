...
class BaseStrategy:
    ...
    def execute_trade(self, date, price, signal, volume):
        ...
        if signal == 'BUY' and self.position <= 0:
            shares = min(max_position, int(volume * 0.1))
            cost = shares * price
...
class MACDStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("MACD策略", initial_capital)
        self.fast = 12
        self.slow = 26
        self.signal = 9
        self.stop_loss = 0.03
...
class KDJStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("KDJ策略", initial_capital)
        self.k_period = 9
        self.stop_loss = 0.03
...
class BollingerStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("布林带策略", initial_capital)
        self.period = 20
        self.std_dev = 2
        self.stop_loss = 0.03
...
class DualMAVolumeStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("双均线量策略", initial_capital)
        self.fast_period = 5
        self.slow_period = 20
...
class MeanReversionStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("均值回归策略", initial_capital)
        self.lookback = 20
        self.std_dev = 2
        self.stop_loss = 0.03
        self.profit_target = 0.05
...
class TrendFollowingStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("趋势跟踪策略", initial_capital)
...
class VolumeBasedStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("量价分析策略", initial_capital)
...
class StatisticalArbitrageStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("统计套利策略", initial_capital)
        self.lookback = 30
...
class EventDrivenStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("事件驱动策略", initial_capital)
...
class QualityRotationStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("质量轮动策略", initial_capital)
        self.rsi_period = 14
...
class RiskParityStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("风险平价策略", initial_capital)
...
class EnhancedHybridStrategy(BaseStrategy):
    def __init__(self, initial_capital=1000000):
        super().__init__("增强混合策略", initial_capital)
        # 参数设置
        self.short_period = 5
...
def main():
    ...
    # 初始化所有策略
    strategies = [
        EnhancedHybridStrategy(initial_capital=1000000),  # 添加新策略
        MACDStrategy(initial_capital=1000000),
        KDJStrategy(initial_capital=1000000),
        BollingerStrategy(initial_capital=1000000),
        DualMAVolumeStrategy(initial_capital=1000000),
        MeanReversionStrategy(initial_capital=1000000),
        TrendFollowingStrategy(initial_capital=1000000),
        VolumeBasedStrategy(initial_capital=1000000),
        StatisticalArbitrageStrategy(initial_capital=1000000),
        EventDrivenStrategy(initial_capital=1000000),
        QualityRotationStrategy(initial_capital=1000000),
        RiskParityStrategy(initial_capital=1000000)
    ]
    ...
    # 在创建策略列表后添加名人策略
    strategies = add_famous_investor_strategies(strategies, data)
    
    # 运行所有策略
    for strategy in strategies:
        run_strategy(strategy, start_date, end_date, data)
        # 确保所有策略在最后一天强制平仓
    ...
    # 打印所有策略的表现
    print("\n" + "="*80)
    print(f"策略对比报告 - {stock_code} {stock_name}")
    print("="*80)
    ...
    # 导出到Excel
    export_to_excel(strategies, stock_code, stock_name, start_date, end_date, data)
